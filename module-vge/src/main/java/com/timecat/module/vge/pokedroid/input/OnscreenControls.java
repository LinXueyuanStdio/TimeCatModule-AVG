package com.timecat.module.vge.pokedroid.input;
...
import com.timecat.module.vge.pokedroid.PokeDroid;
...
public class OnscreenControls implements InputProcessor, Disposable {
    ...
    public OnscreenControls(PokeDroid game) {
        ...
        this.controls.add(new ButtonControl(new Texture(Gdx.files.internal("game/graphics/ui/a_button.png")), new Vector2(32f, 32f), Keys.Z));
        this.controls.add(new ButtonControl(new Texture(Gdx.files.internal("game/graphics/ui/b_button.png")), new Vector2(100f, 32f), Keys.X));
        Texture startTexture = new Texture(Gdx.files.internal("game/graphics/ui/start_button.png")),
                selectTexture = new Texture(Gdx.files.internal("game/graphics/ui/select_button.png"));
        ...
        this.controls.add(new DPADControl(new Texture(Gdx.files.internal("game/graphics/ui/dpad.png")), new Vector2(camera.viewportWidth - 156f, 32f)));
        this.game = game;
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		for (Control c : controls) { if (c.onTouchDown(v.x, v.y, pointer)) { return true; } }
        return false;
    }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        Vector3 v = camera.unproject(new Vector3(screenX, screenY, 0f));
		for (Control c : controls) { if (c.onTouchUp(v.x, v.y, pointer)) { return true; } }
        return false;
    }
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        Vector3 v = camera.unproject(new Vector3(screenX, screenY, 0f));
		for (Control c : controls) { if (c.onTouchDragged(v.x, v.y, pointer)) { return true; } }
        return false;
    }
